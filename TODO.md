...
[X] 로그인은 로그인 화면이 아니라 모달로 띄워서 + 배경은 흐리게
[X] 스낵바 - 우측 상단에 알림 느낌으로 되게
[X] activities 테이블 scroll
[X] 상세 activity
[X] 통계 - 월간 마일스톤 배지 (50km 단위당) , 활동 히트맵 -> 깃허브 or 캘린더 , 최고기록 (최장거리, 최고페이스, 최장시간)
[X] 배지 - badges, user_badges
[X] 하단 푸터 - 소개 | 문의 | 개인정보처리방침 | 이용약관
[X] 마라톤 - 페이지 네이션
[X] 우측 상단 - 알림 + 프로필
[X] 프로필 - 회원 탈퇴 추가
