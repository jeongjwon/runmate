...
const CITIES = ['전체','서울특별시','부산광역시','대구광역시','인천광역시','광주광역시','대전광역시','울산광역시','경기도','강원특별자치도','충청북도','충청남도','전라북도','전라남도','경상북도','경상남도','제주특별자치도']
...
export default function MarathonsPage() {
  ...
  const toggle = (m: Marathon) => {
    if (participationMap[m.id]) {
      showConfirm('내 마라톤에서 제거', `${m.name}을(를) 제거하시겠습니까?`, () => delMut.mutate(m.id))
    } else {
      addMut.mutate(m.id)
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold mb-5 flex items-center gap-2" style={{ color: 'var(--navy)' }}>
        <i className="fas fa-flag-checkered" style={{ color: 'var(--mint)' }} />마라톤 일정
      </h1>

      {/* 필터 */}
      <div className="flex gap-2 mb-5 flex-wrap">
        <select value={city} onChange={e => setCity(e.target.value)} className="input-dark w-auto">
          {CITIES.map(c => <option key={c}>{c}</option>)}
        </select>
        <select value={cat} onChange={e => setCat(e.target.value)} className="input-dark w-auto">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20" style={{ color: 'var(--text2)' }}>
          <i className="fas fa-calendar-times text-4xl mb-3 block" style={{ color: 'var(--border)' }} />
            ...
            return (
              <div key={m.id} className="card p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-bold text-sm leading-snug" style={{ color: 'var(--navy)' }}>{m.name}</h3>
                  {m.isActive
                    ? <span className="badge-mint shrink-0">모집중</span>
                    : <span className="badge-gray shrink-0">마감</span>
                  }
                </div>
                <div className="space-y-1 text-xs mb-3" style={{ color: 'var(--text2)' }}>
                  <div><i className="fas fa-calendar w-4" /> {m.date}</div>
                  <div><i className="fas fa-map-marker-alt w-4" /> {m.location || m.city}</div>
                  {m.entryFee > 0 && <div><i className="fas fa-won-sign w-4" /> {m.entryFee.toLocaleString()}원</div>}
                  {m.categories && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {m.categories.split(',').map(c => (
                        <span key={c} className="badge-gray">{c.trim()}</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="flex gap-2">
                  {m.officialUrl && (
                    <a href={m.officialUrl} target="_blank" rel="noreferrer"
                      className="btn-outline px-3 py-1.5 text-xs font-bold flex-1 text-center">
                      홈페이지
                    </a>
                  )}
                  <button onClick={() => toggle(m)}
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg flex-1 transition-colors ${inMy ? 'text-white' : ''}`}
                    style={inMy
                      ? { background: 'var(--mint)', color: '#fff' }
                      : { border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text2)' }
                    }>
                    {inMy ? <><i className="fas fa-star mr-1" />내 마라톤</> : <><i className="fas fa-plus mr-1" />추가</>}
                  </button>
                </div>
              </div>
            )
