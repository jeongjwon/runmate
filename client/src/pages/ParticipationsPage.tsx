...
export default function ParticipationsPage() {
  ...
  const submitEdit = () => {
    ...
    updateMut.mutate({ marathonId: editTarget.marathonId, body: { finish_time: form.finishTime, race_notes: form.raceNotes } })
  }

  return (
    <div>
      <h1 className="text-2xl font-extrabold mb-5 flex items-center gap-2" style={{ color: 'var(--navy)' }}>
        <i className="fas fa-star" style={{ color: 'var(--mint)' }} />내 마라톤
      </h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20" style={{ color: 'var(--text2)' }}>
          <i className="fas fa-star text-4xl mb-3 block" style={{ color: 'var(--border)' }} />
          <p className="font-bold">참가 중인 대회가 없습니다</p>
          <p className="text-sm mt-1" style={{ color: 'var(--text3)' }}>마라톤 일정에서 대회를 추가해보세요</p>
        </div>
      ) : (
          ...
          {participations.map(p => (
            <div key={p.id} className="card p-4">
              ...
              <div className="space-y-1 text-xs mb-3" style={{ color: 'var(--text2)' }}>
                ...
                {p.raceNotes && (
                  <div className="mt-1 p-2 rounded-lg text-xs" style={{ background: 'var(--surface2)', color: 'var(--text2)' }}>
                    {p.raceNotes}
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  style={{ border: '1px solid var(--border)', background: 'var(--surface)', color: '#ef4444' }}>
      ...
      {/* 완주 기록 편집 모달 */}
      {editTarget !== null && (
        <div onClick={() => setEditTarget(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <h3 className="font-bold mb-4" style={{ color: 'var(--navy)' }}>완주 기록 입력</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold mb-1" style={{ color: 'var(--text2)' }}>완주 시간 (H:MM:SS)</label>
                <input
              ...
              <div>
                <label className="block text-xs font-bold mb-1" style={{ color: 'var(--text2)' }}>레이스 메모</label>
                <textarea
