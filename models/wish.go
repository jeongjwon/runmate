...
type MarathonWish struct {
	...
	Status     string  `json:"status" gorm:"not null"` // wished
...
func (w *MarathonWish) AfterFind(tx *gorm.DB) error {
	if w.FinishTime > 0 {
		w.FinishTimeFormatted = FormatDuration(w.FinishTime)
	}
	return nil
}
