...
export async function POST(req: NextRequest) {
  ...
  // Build header index lookup with aliases
  ...
  const iDist     = colIndex(['distance', 'dist_km', 'distance_km'])
  const iDur      = colIndex(['duration', 'duration_sec', 'time'])
  const iHR       = colIndex(['heart_rate', 'avg_hr', 'heartrate', 'hr'])
  const iCal      = colIndex(['calories', 'cal'])
  const iRoute    = colIndex(['route_type', 'route'])
  const iWeather  = colIndex(['weather'])
  const iNotes    = colIndex(['notes', 'note', 'memo'])
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      // Fall back to positional columns when header lookup fails
  ...
  if (created.length === 0 && errors.length > 0) {
    return NextResponse.json(
      { error: '가져오기 실패', errors },
      { status: 422 }
    )
  }

  return NextResponse.json(
    { message: `${created.length}개 기록 추가됨`, errors },
    { status: 201 }
  )
}
