    ...
    >
      ...
      >
        <div className="px-5 py-4 flex justify-between items-center sticky top-0 bg-[var(--surface)] border-b border-[var(--border)] rounded-t-2xl">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[var(--surface2)] text-[var(--text2)]">
    ...
    <div className="space-y-4">
      ...
      <div className="card p-5">
        ...
        <div className="flex justify-center mb-5">
          <div className="flex gap-0.5 p-1 rounded-xl bg-[var(--surface2)]">
            {(["week", "month", "year", "all"] as Tab[]).map((t) => (
              <button key={t} onClick={() => setTab(t)} className={`act-tab ${tab === t ? "active" : ""}`}>
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-sm font-bold text-[var(--navy)]">{period?.label}</span>
          <button
          ...
          <div className="sm:w-40 shrink-0">
            ...
            <div className="mb-5">
              <div className="flex items-baseline gap-1.5 leading-none">
                <span className="font-black text-[var(--navy)] tracking-tight" style={{ fontSize: "clamp(2.2rem,7vw,3rem)", lineHeight: 1 }}>
                  {stats.dist > 0 ? stats.dist.toFixed(1) : "0"}
                </span>
                <span className="text-sm font-medium text-[var(--text3)]">km</span>
              </div>
            ...
            <div className="space-y-0 divide-y divide-[var(--border)]">
              {[
                ...
                <div key={label} className="flex items-center justify-between py-2.5">
                  <span className="text-[0.65rem] text-[var(--text3)]">{label}</span>
                  <span className="text-sm font-bold text-[var(--navy)]">
                    {val}
                    {unit && <span className="text-[0.65rem] font-normal text-[var(--text3)] ml-0.5">{unit}</span>}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <h2 className="text-xs font-extrabold text-[var(--text2)] uppercase tracking-wider">활동</h2>
            {allRecords.length > 0 && (
              <span className="text-[0.65rem] text-[var(--text3)]">{allRecords.length}개</span>
            )}
          </div>
            ...
            {allRecords.map((r) => {
              ...
              return (
                ...
                >
                  <div className="px-5 py-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex gap-1">
                        {routePts && (
                          <button
                            ...
                            className={`w-6 h-6 flex items-center justify-center rounded-lg text-[10px] transition-colors ${
                              mapOpen ? "text-[var(--mint)]" : "text-[var(--text3)] hover:text-[var(--navy)]"
                            }`}
                          >
                        ...
                        <button
                          onClick={() => showConfirm("기록 삭제", "이 러닝 기록을 삭제하시겠습니까?", () => deleteMut.mutate(r.id))}
                    ...
                    <div className="flex items-center flex-wrap gap-x-2 gap-y-1 text-xs text-[var(--text2)]">
                      <span className="font-medium">{r.duration_formatted}</span>
                      <span className="text-[var(--border)]">·</span>
                      <span className="font-bold text-[var(--mint)]">{r.pace}<span className="font-normal text-[var(--text3)]">/km</span></span>
                      {r.heartRate > 0 && (
                        <>
                          <span className="text-[var(--border)]">·</span>
                    ...
                    {(r.routeType || r.weather || r.notes) && (
                      <div className="flex items-center gap-2 mt-2.5 text-[0.65rem] text-[var(--text3)]">
                        ...
                        {r.routeType && (r.weather || r.notes) && <span>·</span>}
                  ...
                  {/* 루트 지도 */}
                  {mapOpen && routePts && (
                    <div style={{ height: 200 }} className="border-t border-[var(--border)]">
                      <MapContainer center={routePts[0]} zoom={14} style={{ width: "100%", height: "100%" }} scrollWheelZoom={false}>
                        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution='&copy; OpenStreetMap' />
                        <Polyline positions={routePts} color="#00c896" weight={3} opacity={0.85} />
                      </MapContainer>
                    </div>
                  )}
                </div>
