...
const CITIES = [
  '전체','서울특별시','부산광역시','대구광역시','인천광역시','광주광역시','대전광역시',
  '울산광역시','경기도','강원특별자치도','충청북도','충청남도','전라북도','전라남도',
  '경상북도','경상남도','제주특별자치도',
]
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold mb-5 flex items-center gap-2 text-[var(--navy)]">
        <i className="fas fa-flag-checkered text-[var(--mint)]" />마라톤 일정
      </h1>
            ...
            return (
              <div key={m.id} className="card p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-bold text-sm leading-snug text-[var(--navy)]">{m.name}</h3>
                  {m.isActive
                ...
                <div className="space-y-1 text-xs mb-3 text-[var(--text2)]">
                  ...
                  {m.entryFee > 0 && (
                    <div><i className="fas fa-won-sign w-4" /> {m.entryFee.toLocaleString()}원</div>
                  )}
                  {m.categories && (
